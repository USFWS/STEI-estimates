...
\\
\bigskip
\textbf{1} Division of Migratory Bird Management, US Fish and Wildlife
Service, Anchorage, AK, USA, 
\bigskip
...
an approximate 6.53-year period (95\% CI: 5.49, 7.72) with populations
cycling from modelled posterior lows of 25 to 500 individuals and highs
from 230 to \textgreater2000 individuals across the ACP. Over the long
term, the posterior 25-year geometric mean growth rate was -0.02 (CI:
...
eiders \citep{obritschkewitsch2008steller} \citep{wilson2025population}.
It has been hypothesized that these eiders forgo breeding in some years
when when brown lemming (\emph{Lemmus trimucronatus}) abundance and
...
The approach here is inspired by \citep{miller2013} and
\citep{amundson2019}, where generalized additive models
\citep{hastie1986generalized} \citep{wood2017} are used to estimate
...
\citep{sauer2002hierarchical}, \citep{rosenberg2019decline}, and
\citep{smith2021north} among others, although the focus here is only one
species. In general, the advantage of model-based estimates are that
they help to separate noise (i.e., sampling variation) from the true
underlying signal (i.e., temporal or spatial patterns in density,
\citep{BDA3}, \citep{hooten2015guide}, many others) and is closely
related to the concept of shrinkage of random effects
\citep{efron1977stein}, \citep{BDA3}, where information is shared across
...
citations above, and \citep{kery2015applied}, \citep{kery2020applied}).
...
2019 and 2022 to 2024) are used. \citep{amundson2019} also included data
from a different survey (before 2007) with different seasonal timing
that causes a difference in observed response (a major subject of the
effort in \citep{amundson2019}). Data collected before 2007 have also
not undergone the same level of quality control, which was a major
effort for this analysis. The ACP data from 2007 to 2024 used in this
report, a description of quality control processes, and a description of
data manipulations are available at \citep{osnas-github}
\citep{osnasACP} (see \nameref{s1-acpdata}).

The Triangle survey is described in reports from ABR
\citep{obritschkewitsch2008steller} \citep{triangleABR}. Data for the
...
\caption{\label{fig-area}Arctic Coastal Plain (ACP) and Triangle survey
...
four year period to increase spatial coverage (not shown).}
...
for strip-transect surveys \citep{cochran1977} \citep{williams2002}
\citep{thompson2012}, was investigated but found to be poor due to the
distribution of observations and the triangle shape of the survey area,
where most observations are on short transects in the north and longer
transects in the south rarely have observed animals. This nullifies any
advantage of a ratio estimator, which can produce a better estimate when
the transect length and number of encounters are positively related
\citep{cochran1977}, \citep{thompson2012}. The point estimate was
calculated as
...
et al. \citep{fewster2009} showed that the above can overestimate the
variance of systematic surveys when there is a strong gradient in
density, as there is in this area. In any case, the estimator above is
much better than a standard ratio estimator in this specific case.

\subsection{Model-based estimates}\label{model-based-estimates}

I used generalized additive models (GAMs, \citep{hastie1986generalized},
\citep{wood2017}, \citep{pedersen2019hierarchical}) to estimate eider
density as a function of location and time. A GAM can be thought of as a
generalized linear mixed model that fits smooth functions (splines) of
covariates to predict the response, in addition to standard linear mixed
model terms if specified. The optimal degree of smoothing is determined
during model fitting through a model selection-like process. The
smoothed temporal or spatial effect, thus, can account for
autocorrelation in these domains \citep{wood2017},
\citep{pedersen2019hierarchical}, \citep{hefley2017basis}. GAMs are
widely used in animal density surface modeling (e.g.,
\citep{miller2013}, \citep{amundson2019}, \citep{hollmen2023climate},
and many others).
...
(pairs and males) were summed for each segment and the coordinates (in
isomorphic UTMs) of the centroid were used for spatial location. Year
was used as the covariate for temporal effects. The same procedure was
used for both the entire ACP and for the Triangle survey, but for the
ACP-only model (see below) the segment size for model fitting was 6 km.
This was done for computational reasons (time) related to fitting many
different models and to be consistent with \citep{amundson2019}. When
Triangle and ACP data were combined into one model, a common 1 km
segment size was used. All spatial data manipulation was done using the
R package \texttt{sf} \citep{sf}.
...
product smooth'' (a multidimensional smooth that allows the units for
dimensions to differ), and \(s_{re}()\) is a random effect in the usual
...
used a a `thin plate regression spline' (\texttt{s(...,\ bs\ =\ "tp")}
...
appropriately modeled the large number of response zeros in these data.
To select the best model, the \texttt{mgcv::AIC} function was used.
...
\citep{r-mgcv}, \citep{wood2017} to predict the expected density of
...
distribution of model parameters \citep{wood2017}
\citep{miller2025bayesian}. Therefore, posterior simulation was used to
calculate population totals over the study area for each year (see
examples in help files for \texttt{mgcv::predict} in \citep{r-mgcv} or
\citep{wood2017}, p.~342-343). Predictions were made once on the same
grid and years as described above but \texttt{type\ =\ lpmatrix} was
specified so that a design matrix, \(\textbf X\), was returned with one
row for each prediction location-year and one column for each term in
the model. Multivariate normal samples of the model parameters,
\(\textbf b_i\) were then simulated using the fitted model parameter
vector and variance-covariance matrix. For the Triangle study area,
direct simulation from a multivariate normal distribution was used. For
all simulations on the ACP study area, a Metropolis-Hastings algorithm
was used to obtain samples because large areas of zero observations
caused poor performance based on direct multivariate normal simulation
using the parameter estimates and covariance matrix (see
...
transect half-strip width (Table 3 from \nameref{s4-bradley}). In this
...
estimate over each of the four distance bins in Table 3 of
\nameref{s4-bradley}. Because distance bins were of equal width,
detection was estimated as a factor of bin (i.e, a continuous distance
function was not estimated), and no other covariates were found
important, the mean and variance are a simple equally weighted average
over the distance bins. In Table 3 of \nameref{s4-bradley} only the mean
and ninety-five percent credible interval was given, so I approximated
the standard error of the estimate in each bin by the range divided by
2*1.96, which is the typical number of standard deviations in the range
of a symmetric credible interval.
...
\caption{\label{fig-observations}Observed Steller's eider during the
Arctic Coastal Plain (2007-2024) and Triangle (1999-2023) surveys.}

\end{figure}%

\begin{figure}

\caption{\label{fig-count}Number of Steller's eider observations by year
for the Arctic Coastal Plain and Triangle surveys.}
...
detection bias and, presumably less often, species miss-identification.
...
have been the case in 2009 on the Triangle area (Fig~\ref{fig-design}
A)and in 2010 for all strata of the ACP (Fig~\ref{fig-design} B), and
was the case in 2, 10, 16 and 16 years out of 16 for the High,
Teshekpuk, and Medium, and Low strata of the ACP, respectively.

\begin{figure}

\caption{\label{fig-design}Design-based estimates in the Triangle survey
...
\caption{\label{tbl-modelsTri}AIC table for models fit to Triangle data.
Model structures are described in the main text.}
...
\toprule
Model & Linear Predictor & df & AIC & DeltaAIC \\ 
...
degrees of freedom for the spatial, temporal, and spatio-temporal smooth
...
smooth (Fig~\ref{fig-model-3}), density decreased through time in the
...
trend (Fig~\ref{fig-model-2}).

\begin{figure}

\begin{minipage}{0.45\linewidth}

\subcaption{\label{fig-model-1}Spatial smooth}
...
\subcaption{\label{fig-model-2}Temporal smooth}
...
\subcaption{\label{fig-model-3}Spatio-temporal smooth}

\end{minipage}%

\caption{\label{fig-model}Plots of spatial (A), temporal (B), and
...
\caption{\label{fig-densityABR}Predicted average density of Steller's
eider after removing the effect of year and space-by-year intereaction
in the Triangle survey area.}

\end{figure}%

Population estimates across the Triangle survey area by year for
indicated breeding birds without (A) and with (B) a detection correction
are shown in Fig~\ref{fig-yearABR}. There is a substantial increase in
the population estimates, the uncertainty, and in the skew of the
posterior toward higher population estimates with the application of the
a detection correction (Fig~\ref{fig-abrB}). This is due to the low mean
detection rate and high uncertainty in the detection prior. Note that
the model predicts the population in 2020 when no survey was conducted.
The population appears to cycle on a period of 5-7 years
(Fig~\ref{fig-abrB}); thus, causing the posterior n-year trend to
fluctuate at the posterior mean (Fig~\ref{fig-trendABR}). Trend
estimates are more precise for longer lags, but also fluctuate from
positive to negative at the posterior mean (Fig~\ref{fig-trendABR}).

\begin{figure}

\begin{minipage}{\linewidth}

\subcaption{\label{fig-abrA}}

\end{minipage}%
\newline
\begin{minipage}{\linewidth}

\subcaption{\label{fig-abrB}}

\end{minipage}%

\caption{\label{fig-yearABR}Posterior estimates of Steller's eider in
...
\caption{\label{fig-trendABR}Posterior trend estimates for Steller's
...
(Fig~\ref{fig-modelACP-1}). Despite the shrinkage of the temporal random
effects, they still varied greatly on the log scale
(Fig~\ref{fig-modelACP-2}).

\begin{table}
\begin{adjustwidth}{-2.25in}{0in}
\caption{\label{tbl-modelsACP}AIC table for model fit to Arctic Coastal
...
\subcaption{\label{fig-modelACP-1}Spatial smooth}

\end{minipage}%
\newline
\begin{minipage}{\linewidth}

\subcaption{\label{fig-modelACP-2}Temporal effects}

\end{minipage}%

\caption{\label{fig-modelACP}Partial effect of (A) spatial smooth and
...
are shown in Fig~\ref{fig-yearACP} without a detection correction (A)
and with a detection correction (B). The population appears to fluctuate
in a similar manner as in the model fit to Triangle survey data, but the
year-specific estimates for the ACP are much less precise. Because the
best model contained only a simple random effect and the survey was not
completed in 2020 and 2021, predictions were not made for these years
using this model. Such predictions could be made by simulating random
effects for these years but this would simply give an estimate that
spans the range of historical estimates because there is no temporal
...
\subcaption{\label{fig-acpA}}

\end{minipage}%
\newline
\begin{minipage}{\linewidth}

\subcaption{\label{fig-acpB}}

\end{minipage}%

\caption{\label{fig-yearACP}Posterior estimates of Steller's eider in
...
the ACP based on model \(M4\) (Fig~\ref{fig-acpA}) are less extreme than
those based on design-based estimates (Fig~\ref{fig-design} B). That is,
the model-based estimates are lower in high abundance years and higher
...
(CI: -0.03, 0.23), which could be a modest decrease to a very fast
increase (cf., Fig~\ref{fig-acpB}). For all time periods, the 95\%
credible interval overlaps 0 and the upper bound was often
\textgreater{} 0.15 (Fig~\ref{fig-trendACP}).

\begin{figure}

\caption{\label{fig-trendACP}Posterior trend estimates for Steller's
...
(\textless\textless{} 0.001) and the survey crew effect p-value was
...
distance from the coast (Fig~\ref{fig-combPart}). Similar to the
Triangle model, the temporal trend fluctuated with a 5-7 year period
(Fig~\ref{fig-temp}). The survey effect was estimated at 0.46 (SE =
...
Fig~\ref{fig-combZoom}. Because there is no space-time interaction in
...
\subcaption{\label{fig-combPart}}

\end{minipage}%
%
\begin{minipage}{0.50\linewidth}

\subcaption{\label{fig-temp}}

\end{minipage}%
\newline
\begin{minipage}{\linewidth}

\subcaption{\label{fig-combZoom}}

\end{minipage}%

\caption{\label{fig-mapComb}Partial effect plots for the spatial (A) and
...
over the entire ACP area are shown in Fig~\ref{fig-combA} without a
detection correction and in Fig~\ref{fig-combB} after the detection
...
\subcaption{\label{fig-combA}}

\end{minipage}%
\newline
\begin{minipage}{\linewidth}

\subcaption{\label{fig-combB}}

\end{minipage}%

\caption{\label{fig-yearComb}Posterior estimates of Steller's eider in
...
\caption{\label{fig-trendComb}Posterior trend estimates for Steller's
...
15 year period. When the spectorgram power is averaged across all years
...
simulation pvalue of p = 0.07, suggesting very weak evidence for a
longer period ocilation. When 500 posterior samples of the time series
were sampled from the GAM and a spectogram found for each sample, the
posterior maximum power was found at period (6.52) with credible
interval (CI: 6.23, 6.76), suggesting very high confidence in maximum
average power at this period across the range of posterior sample of the
GAM.

\begin{figure}

\caption{\label{fig-wavelet}Wavelet power spectrum for Steller's eider
...
approximate integer multiples of the period (e.g., 6.5, 13.0, 19.6, and
so on) so that similar phases of the cycle are compared. For example, a
19-year trend calculated from 2003 to 2022 gives a posterior mean of
0.03 (CI: -0.01, 0.09).
...
size, populations trends, or variation in density across space, then
...
applied for trend estimation (e.g. \citep{amundson2019},
\citep{rosenberg2019decline}, \citep{smith2021north}). Another advantage
of the linear models used here (GAMs) is that interpretation and
understanding is straightforward, where effects can be partitioned out
into time, space, and space-by-time interaction effects (e.g.,
Fig~\ref{fig-model}).
...
Fig~\ref{fig-densityABR} to Fig~\ref{fig-combZoom}. In addition, the
...
south through time (Fig~\ref{fig-model-3}). The model also estimates the
temporal effect well with much better precision than the design-based
estimates (compare Fig~\ref{fig-abrA} to Fig~\ref{fig-design} a).
...
the combined data. Thus, combining data sets allow the Triangle data to
...
Fig~\ref{fig-yearACP} shows that relatively few eiders are outside the
Triangle area in most years, even though there are observations outside
the Triangle in some years (Fig~\ref{fig-observations}). Because the
space-time interaction of the Triangle-only model shows a decreasing
trend in the south (Fig~\ref{fig-model-3}), there may be fewer eiders
...
The model and approach used here is similar to that of
...
\citep{sauer1994observer} \citep{fink2023double}. Therefore, observer
...
One interesting comparison to \citep{amundson2019} is that they found
very linear time trends for Steller's eider. This was also the case here
when models included a spatio-temporal interaction and were fit to
ACP-only data (model M2, Table~\ref{tbl-modelsACP}, results in
...
more complex model when confronted with sparse or miss-matched data in
time and space.
...
for at least 25 years. Thus, any measure of trend will show an
increasing, decreasing, or stable estimate depending on the beginning
and ending points for the trend calculation (Fig~\ref{fig-trendABR},
Fig~\ref{fig-trendComb}). Recovery criteria and species status
...
for trends so that similar points in the cycle are used (multiples of
approximately 6.5). While each cycle is a stochastic realization that
...
period but perhaps varying amplitude.
...
estimates for long-tailed duck and did not account for non-detection by
both observers (a `00' capture history). Analyses previous to 2019 (Bob
Stehn, unpublished) used a point estimate of 0.3 (SD = 0) based on
expert judgement, which is remarkably close to the mean estimate used
here, but did not include any uncertainty. Dunham and Grand
\citep{dunham2017evaluating} used the same point estimate and allowed
for some uncertainty (SD = 0.02). Therefore, the posterior mean
...
\url{https://www.sciencebase.gov/catalog/item/6761f392d34e9734dd458940}.
...
\paragraph*{S4 Detection}
\label{s4-bradley}
{\textbf{A description of the field experiment and analysis to estimate
detection in 2017 and 2018.}}
...
\url{https://www.sciencebase.gov/catalog/item/6761f393d34e9734dd458942}.}}
