...
author:
  ...
  affiliation: 
    ...
    department: \newline US Fish and Wildlife Service
    city: Anchorage
    state: AK
date: "`r Sys.Date()`"
...
execute: 
  ...
  warning: false
abstract: |
  Annual populations and spatial variation in density is estimated across the Arctic Coastal Plain (ACP) and Utqiaġvik Triangle (Triangle) survey areas for the Steller's Eider (*Polysticta stelleri*) using spatio-temporal generalized additive models and contrasted to more traditional design-based estimates. Compared to design-based estimates, model based estimates were more precise, less variable between years,  fine-scale spatial density maps can easily be produced, and the two data sets can be easily combined to produce a common populations estimate. Across both data sets and when combined, Steller's Eider populations flucuated with a 6-7 year period with population posterior lows that ranged near zero to 200 and highs from 200 to >1000 across the ACP. Over the long term, the posterior 25-year geometric mean growth rate was -0.01 (95% CI: -0.06, 0.04) with shorter term growth rate less well estimate and fluated between positive and near zero. Density maps showed that highest density concentration is in the northern part of the Triangle area with eider much less common southward and across other areas of the ACP. MOdels fit to just the TRiangle area, shown that eider density has been shifting northward though time, but this patterns was not support in the much sparser ACP data set, nor was it well estimated when the ACP and Tinagle data sets wrere combined.    
---
