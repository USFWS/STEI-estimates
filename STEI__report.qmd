---
title: "Steller's Eider (*Polysticta stelleri*) population and density estimates from the Arctic Coastal Plain and Utqiaġvik Triangle surveys using generalized additive models"
author:
  name: Erik E. Osnas
...
execute: 
  ...
  warning: false
abstract: |
  Annual populations and spatial variation in density was estimated across the Arctic Coastal Plain (ACP) and Utqiaġvik Triangle (Triangle) survey areas for Steller's Eider (*Polysticta stelleri*) using spatio-temporal generalized additive models and compared to traditional design-based estimates. Compared to design-based estimates, model based estimates were more precise, less variable between years,  fine-scale spatial density maps can be produced, and the data sets can be combined to produce a common populations estimate. Across both data sets and when combined, Steller's Eider populations flucuated with an approximate 6 year period with population lows ranging from zero to 200 and highs from 200 to >1000 individuals across the ACP. Over the long term, the posterior 25-year geometric mean growth rate was -0.01 (95% CI: -0.06, 0.04) with shorter term growth rate less well estimated and fluctuating between positive and near zero. Mean population size across the ACP over the last 20 years was 230 (CI: 120, 437) under a model that combined both data sets. Density maps showed a high density concentration in the northern part of the Triangle area and lower densities southward and across the ACP. Models fit to just the Triangle area, indicated that eider density has been shifting northward though time, but this pattern was not supported in the much sparser ACP data set, nor was it well estimated when the ACP and Tinagle data sets were combined. 
...
```{r}
#| label: fig-designACP
#| fig-cap: "Design-based estimates in the Arctic Coastal Plain survey area. Confidence intervals have been truncated at zero. No flying birds are recorded in this survey. No detection correction was applied."
...
Partial effect plots for the spatial (a) and temporal (b) smooth from a GAM model fit to the Triangle and ACP data combined. Spacial prediction od density for the Triangle area in 2013 (c). 
:::
...
```{r}
#| label: fig-deriv
#| fig-cap: First derivative of the temporal smooth on the link scale from the model for the ACP using the combined data. The shaded region gives 2 standard errors from the mean derivative, and showns regions where the deriviative is clearly positive (an increasing population) or negative (a decreasing population). A minimum or maximum in the cycle is indicated where the derivative crosses zero.
#| eval: false
...
```{r}
#| label: fig-wavelet
#| fig-cap: Wavelet power spectrum (a) for Steller's Eider across the ACP based on the posterior mean of the total population. White lines gives a 95% confidence interal. Larger power level (red) corespond to greater "matching" of the wave period. Average power spectrum across years (b) with statistical significance level shown in colors.
#| layout-nrow: 2
#| fig-subcap: 
#|   - "Power spectrum"
#|   - "Average wavelength"
...
#should probably sample over poaterior to find CI
...
One interesting comparison to Amundson et al. (2019) is that for Steller's Eider they found very linear time trends. This was also the case for models fit here when a spatio-temporal interaction was included (results not shown). Such a model was not used here because it gave widely unbelievable predictions from 1999 to 2006 in the case where ACP and Triangle data were combined. I attributed this to the spatio-temporal mis-match in data sets during this period, and perhaps a spatio-temporal mis-match in sampling intensity. This was not a problem in the data of Amundson et al. (2019), but they did not fit simpler models without a space-time interaction. Because the simpler model without a space-time interaction reported here produced reasonable predictions and all other data sets produced similar cyclic temporal patterns, I believe the simpler model with temporal and spatial effects is a more appropriate model. This might change with more data or different analysis. In any case, a perfectly straight temporal trend is hard to believe, and most likely represents a modelling artifact in this more complex model. It would be interesting to test if the linear trend remains when the time series is truncated so that both data sets matched in temporal extent. If both gave similar predictions to the simpler model fit to the full data, then the justification for using the simple model to extend the temporal extent of inference would be strengthened.  

Previous modelling efforts for population and trend estimation reported in the last Species Status Assessment [SSA, @steiSSA2019] used a Poisson-Binomial hierarchical or state-space model, where the true unobserved population (aka, state) was modeled as a Poisson random variable with an AR1-type temporal autocorrelation structure for the rate parameter but no spatial effects, and the observed data (response) was modeled as a binomial random variable with parameters $p$ (the detection probability) and $N$, the size of the population in the sampled area (the state). In the case of the Triangle area, a model was fit to the summed total number of eiders observed across all transects for each year because transect-specific counts were not available. Thus, inter-transect variance cannot be estimated and the only reasonable approach was to assume a Poisson-Binomial response. While this may approximate the mean response, there was no way to assess this model for the Triangle area because there was only one observation each year. In effect, the there was no information in the data to separate the observation process from the state process, and the model served only to incorporate detection to estimate population size and (linear) trend. The model used in 2019 for the ACP was essentially identical in that it ignored spatial variation and used a Poisson state process and binomial observation process, but the response was transect-specific with a constant mean across all transects. While both models seem to capture basic temporal patterns, the approach used here is an improvement as the negative binomial likelihood adequately described the variation in response after spatial and temporal effects were accounted for by the linear predictor. Both models can be thought of as modeling the same expected response, $p \lambda A$, where $p$ is the detection rate, $A$ is the area over which an observation is made, and $\lambda$ is a rate parameter. In the Binomial-Poisson model used in 2019, the variation of the response (the conditional likelihood) is specified by the analyst as it depends on the Poisson assumption that the variance equals the mean and depends on the prior for $p$. In the negative binomial model used here, the expectation $p \lambda A$ is also estimated but the scale parameter of the negative binomial (the "variance" of the likelihood function) is directly estimated instead of specified by the analyst. Detection is then removed to estimate $\lambda A$ during simulation after estimation of model parameters, which included the spatial and temporal effects. In this sense, the current approach is better than that used in 2019. 

Steller's Eider populations on the ACP, including the Triangle area, are clearly undergoing cyclical patterns of abundance with a period of about 6.39 years (@fig-yearABR, @fig-yearComb, @fig-wavelet). While it has been hypothesized that this is due to lemming cycles [@quakenbush2004breeding], this effort cannot determine the cause of the cycles. It is clear that cycles exist and that the population has been fluctuating around (apparently) stable cycles for some at least 25 years. Thus, any measure of trend will show an increasing, decreasing, or stable estimate depending on the beginning and ending points for the trend calculation (@fig-trendABR, @fig-trendComb). Recovery criteria and species status assessment should incorporate this understanding into status determination. Future work might attempt to partition cyclical from directional trends in the time series to understand overall population dynamics, but the long term geometric means reported here do average over approximately 3.75 of these cycles (@fig-trendComb). Attention should be given the the start and end points for trends so that similar points in the cycle are used (multiples of approximately 6.39).    
...
Osnas, E.E. 2024. Steller's Eider (*Polysticta stelleri*) population and density estimates from the Arctic Coastal Plain and Utqiaġvik Triangle surveys using generalized additive models. US Fish and Wildlife Service, Division of Migratory Bird Management, Anchorage, AK. Preview version. 
