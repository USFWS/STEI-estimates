...
\begin{flushleft} \DIFaddFL{Model structures are described in the main text.
}\end{flushleft}
...
degrees of freedom for the spatial \DIFaddbegin \DIFadd{(Fig~\ref{fig-model}A), temporal (Fig~\ref{fig-model}B)}\DIFaddend , \DIFdelbegin \DIFdel{temporal, }\DIFdelend and spatio-temporal \DIFaddbegin \DIFadd{(Fig~\ref{fig-model}C) }\DIFaddend smooth
were 16.1, 12.1, and 2.1\DIFaddbegin \DIFadd{, }\DIFaddend respectively. Highest densities of Steller's
...
smooth (Fig~\DIFdelbegin \DIFdel{\ref{fig-model-3}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-model}C}\DIFaddend ), density decreased through time in the
...
%DIF < 
%DIF < 
%DIFDELCMD < \begin{minipage}{0.10\linewidth}
%DIFDELCMD < %%%
\DIFdelFL{~}%DIFDELCMD < \end{minipage}%%%
%DIF < 
%DIF < 
...
{%DIFAUXCMD
%DIFDELCMD < \label{fig-model-2}%%%
\DIFdelFL{Temporal smooth}}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIF < 
%DIFDELCMD < \newline
%DIFDELCMD < \begin{minipage}{\linewidth}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIFDELCMD < \subcaption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-model-3}%%%
\DIFdelFL{Spatio-temporal smooth}}
%DIFAUXCMD
%DIFDELCMD < 
...
\DIFdelFL{Plots of spatial (A), temporal (B), and
spatio-temporal interaction (C) partial effects from the best fitting
model. In (B) the shaded region is 2 standard errors. In (A) and (C)
black lines show contour lines of equal partial effect, and color shows
increased (red) or decreased (blue) density relative to the intercept.
In (C) the interaction is shown by 16 density surfaces at different time
points from 1999 to 2023. Each panel is as in (A) but has been
simplified to the bounding rectangle of the survey area. Note that as
time progresses, the upper left of each panel increases in density as
indicated by the shift from blue to red.}}
%DIFAUXCMD
%DIFDELCMD < 
...
\caption{\label{fig-densityABR}\DIFdelbeginFL \DIFdelFL{Predicted average density of Steller's
eider after removing the effect of year and space-by-year intereaction
in the Triangle survey area.}\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Predicted average density of Steller's eider in the Triangle survey.}}\newline{Predicted density after the effects of year and space-by-year interaction have been removed.}\DIFaddendFL }

\end{figure}%

Population estimates across the Triangle survey area by year for
indicated breeding birds without (\DIFaddbegin \DIFadd{Fig~\ref{fig-yearABR}}\DIFaddend A) and with (\DIFaddbegin \DIFadd{Fig~\ref{fig-yearABR}}\DIFaddend B) a detection correction are shown in Fig~\ref{fig-yearABR}. There is a substantial increase in the population estimates, the uncertainty, and in the skew of the posterior toward higher population estimates with the application of the a detection correction (Fig~\DIFdelbegin \DIFdel{\ref{fig-abrB}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-yearABR}B}\DIFaddend ). This is due to the low mean detection rate and high uncertainty in the detection prior. Note that the model predicts the population in 2020 when no survey was conducted. The population appears to cycle on a period of 5-7 years (Fig~\DIFdelbegin \DIFdel{\ref{fig-abrB}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-yearABR}B}\DIFaddend ); thus, causing the posterior n-year trend to fluctuate at the posterior mean (Fig~\ref{fig-trendABR}). Trend estimates are more precise for longer lags, but also fluctuate from positive to negative at the posterior mean (Fig~\ref{fig-trendABR}).

\begin{figure}

\DIFdelbeginFL %DIFDELCMD < \begin{minipage}{\linewidth}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \caption{\label{fig-yearABR}\textbf{\DIFaddFL{Posterior estimates of Steller's eider in the Triangle survey area, 1999-2023.}}\newline{(A) without accounting for detection and (B) after applying a detection correction. The black line is the posterior mean, the dashed black line is the posterior median, and the orange band is the 95\% credible interval.}}
\DIFaddendFL 

\DIFdelbeginFL %DIFDELCMD < \subcaption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-abrA}%%%
}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-yearABR}%%%
\DIFdelFL{Posterior estimates of Steller's eider in
the Triangle survey area without accounting for detection (A) and after
applying a detection correction (B). The black line is the posterior
mean, the dashed black line is the posterior median, and the orange band
...
\caption{\label{fig-trendABR}\DIFdelbeginFL \DIFdelFL{Posterior trend estimates for Steller's
eider in the Triangle survey area, 1999-2023. The y axis is the log of
the geometric mean growth rate, and the x axis is the lag-year trend,
i.e., for lag 10 gives the 10-year trend from 2013 to 2023. The black
line is the posterior mean, the dashed black line is the posterior
median, and the orange band is the 95\% credible interval. The
horizontal thick black line is the y-axis origin.}\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Posterior trend estimates for Steller's eider in the Triangle survey area, 1999-2023.}} \newline{The y axis is the log of the geometric mean growth rate, and the x axis is the lag-year trend, the lag of 10 gives the 10-year trend from 2013 to 2023. The black line is the posterior mean, the dashed black line is the posterior median, and the orange band is the 95\% credible interval. The
horizontal thick black line is the y-axis origin.}\DIFaddendFL }
...
(Fig~\DIFdelbegin \DIFdel{\ref{fig-modelACP-1}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-modelACP}}\DIFaddend ). Despite the shrinkage of the temporal random
effects, they still varied greatly \DIFdelbegin \DIFdel{on the log scale
(Fig~\ref{fig-modelACP-2}}\DIFdelend \DIFaddbegin \DIFadd{(see below}\DIFaddend ).

\begin{table}
\begin{adjustwidth}{-2.25in}{0in}
\caption{\label{tbl-modelsACP}\DIFdelbeginFL \DIFdelFL{AIC table for model fit to Arctic Coastal
Plain data. Model structures are described in the main text. The suffix
`.obs' indicates a model that contained a random effect of observer.}\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{AIC table for model fit to Arctic Coastal Plain data.}}\DIFaddendFL }
...
%DIFDELCMD < Model & Linear Predictor & df & AIC & DeltaAIC \\ 
%DIFDELCMD < \midrule\addlinespace[2.5pt]
%DIFDELCMD < M4 & s(X,Y) + s(fYear) & 23.65 & 531.58 & 0.00 \\ 
%DIFDELCMD < M4.obs & s(X,Y) + s(fYear) + s(Observer) & 23.65 & 531.58 & 0.00 \\ 
%DIFDELCMD < M3 & s(X,Y) + s(Year) + s(fYear) & 24.51 & 531.83 & 0.25 \\ 
%DIFDELCMD < M3.obs & s(X,Y) + s(Year) + s(fYear) + s(Observer) & 24.53 & 531.86 & 0.28 \\ 
%DIFDELCMD < M0 & s(X,Y) & 14.54 & 537.44 & 5.86 \\ 
%DIFDELCMD < M0.obs & s(X,Y) + s(Observer) & 14.54 & 537.44 & 5.86 \\ 
%DIFDELCMD < M1 & s(X,Y) + s(Year) & 15.60 & 539.22 & 7.64 \\ 
%DIFDELCMD < M1.obs & s(X,Y) + s(Year) + s(Observer) & 15.60 & 539.22 & 7.64 \\ 
%DIFDELCMD < M2 & s(X,Y) + s(Year) +ti(X,Y,Year) & 14.07 & 540.47 & 8.89 \\ 
%DIFDELCMD < M2.obs & s(X,Y) + s(Year) + ti(X,Y,Year) + s(Observer) & 14.07 & 540.47 & 8.89 \\ 
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular*}
%DIFDELCMD < 
...
\begin{flushleft}  \DIFaddFL{Model structures are described in the main text. The suffix
...
%DIFDELCMD < \subcaption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-modelACP-2}%%%
\DIFdelFL{Temporal effects}}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-modelACP}%%%
\DIFdelFL{Partial effect of (A) spatial smooth and
(B) year random effects for the Arctic Coastal Plain model M3. The map
of density (A) shows decreasing density further to the south or away
from the coast.}}
%DIFAUXCMD
%DIFDELCMD < 
...
are shown in Fig~\ref{fig-yearACP} without a detection correction (\DIFaddbegin \DIFadd{Fig~\ref{fig-yearACP}}\DIFaddend A) and with a detection correction (\DIFaddbegin \DIFadd{Fig~\ref{fig-yearACP}}\DIFaddend B). The population appears to fluctuate in a similar manner as in the model fit to Triangle survey data, but the year-specific estimates for the ACP are much less precise. Because the best model contained only a simple random effect \DIFaddbegin \DIFadd{to model year }\DIFaddend and the survey was not completed in 2020 and 2021, predictions were not made for these years using this model. Such predictions could be made by simulating random effects for these years but this would simply give an estimate that spans the range of historical estimates because there is no temporal correlation in this model.

\begin{figure}

\DIFdelbeginFL %DIFDELCMD < \begin{minipage}{\linewidth}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \caption{\label{fig-yearACP}\textbf{\DIFaddFL{Posterior estimates of Steller's eider across the Arctic Coastal Plain survey area, 2007-2024.}} \newline{(A) without accounting for detection and (B) after applying a detection correction. The black line is the posterior mean, the dashed black line is the posterior median, and the orange band is the 95\% credible interval.}}
\DIFaddendFL 

\DIFdelbeginFL %DIFDELCMD < \subcaption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-acpA}%%%
}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-yearACP}%%%
\DIFdelFL{Posterior estimates of Steller's eider in
across the ACP survey area without accounting for detection (A) and
after applying a detection correction (B). The black line is the
posterior mean, the dashed black line is the posterior median, and the
orange band is the 95\% credible interval.}}
%DIFAUXCMD
%DIFDELCMD < 
...
the ACP based on model \(M4\) (Fig~\DIFdelbegin \DIFdel{\ref{fig-acpA}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-yearACP}}\DIFaddend ) are less extreme than those based on design-based estimates (Fig~\ref{fig-design}B). That is, the model-based estimates are lower in high abundance years and higher
...
(\DIFaddbegin \DIFadd{95\% }\DIFaddend CI: -0.03, 0.23), which could be a modest decrease to a very fast
increase (cf., Fig~\DIFdelbegin \DIFdel{\ref{fig-acpB}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-yearACP}B}\DIFaddend ). For all time periods, the 95\%
credible interval overlaps 0 and the upper bound was often
\textgreater{} 0.15 (Fig~\ref{fig-trendACP}).

\begin{figure}

\caption{\label{fig-trendACP}\DIFdelbeginFL \DIFdelFL{Posterior trend estimates for Steller's
eider in the Arctic Coastal Plain survey area, 2007-2024. The y axis is
the log of the geometric mean growth rate, and the x axis is the
lag-year trend, i.e., for lag 10 gives the 10-year trend from 2014 to
2024. The black line is the posterior mean, the dashed black line is the
posterior median, and the orange band is the 95\% credible interval. The
horizontal thick black line is the y-axis origin.}\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Posterior trend estimates for Steller's eider in the Arctic Coastal Plain survey area, 2007-2024.}} \newline{The y axis is the log of the geometric mean growth rate, and the x axis is the lag-year trend, the lag of 10 gives the 10-year trend from 2014 to 2024. The black line is the posterior mean, the dashed black line is the posterior median, and the orange band is the 95\% credible interval. The horizontal thick black line is the y-axis origin.}\DIFaddendFL }
...
(\DIFdelbegin \DIFdel{\textless\textless{} }\DIFdelend \DIFaddbegin \DIFadd{< }\DIFaddend 0.001) and the survey crew effect p-value was
...
distance from the coast (Fig~\DIFdelbegin \DIFdel{\ref{fig-combPart}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-mapComb}A}\DIFaddend ). Similar to the
Triangle model, the temporal trend fluctuated with a 5-7 year period
(Fig~\DIFdelbegin \DIFdel{\ref{fig-temp}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-mapComb}B}\DIFaddend ). The survey effect was estimated at 0.46 (SE =
...
Fig~\DIFdelbegin \DIFdel{\ref{fig-combZoom}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-mapComb}C}\DIFaddend . Because there is no space-time interaction in
...
(Fig~\DIFdelbegin \DIFdel{\ref{fig-temp}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-mapComb}B}\DIFaddend ).

\begin{figure}

\DIFdelbeginFL %DIFDELCMD < \begin{minipage}{0.50\linewidth}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \caption{\label{fig-mapComb}\textbf{\DIFaddFL{Partial effect plots for the best model fit to the combined Triangle and Arctic Coastal Plain data.}} \newline{Partial effect plots for (A) spatial and (B) temporal effects. There is no space-time interaction in this model. (C) spatial prediction of eider density in 2013 zoomed in to the Triangle area with no
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-mapComb}%%%
\DIFdelFL{Partial effect plots for the spatial (A) and
temporal (B) smooth from a GAM model fit to the Triangle and Arctic
Coastal Plain data combined. There is no interaction in this model.
Spatial prediction of eider density in 2013 zoomed in to the Triangle
area (C). Shown in (C) is the the indicated breeding bird index (no
detection correction). Because there is no space-time interaction,
relative densities are the same across years. Similarly, the detection
correction does not vary across years or other covariates, the relative
surface does not change with or without a detection correction.}}
%DIFAUXCMD
%DIFDELCMD < 
...
over the entire ACP area are shown in Fig~\DIFdelbegin \DIFdel{\ref{fig-combA} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-yearComb}A }\DIFaddend without a
detection correction and in Fig~\DIFdelbegin \DIFdel{\ref{fig-combB} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-yearComb}B }\DIFaddend after the detection
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-yearComb}%%%
\DIFdelFL{Posterior estimates of Steller's eider in
the ACP survey area with both Arctic Coastal Plain and Triangle data
combined for model fitting. Posteriors are given without accounting for
detection (A) and after applying a detection correction (B). The black
line is the posterior mean, the dashed black line is the posterior
median, and the orange band is the 95\% credible interval.}}
%DIFAUXCMD
%DIFDELCMD < 
...
\caption{\label{fig-trendComb}\DIFdelbeginFL \DIFdelFL{Posterior trend estimates for Steller's
eider in the ACP survey area using data from both the Triangle and ACP
surveys, 1999-2024. The y axis is the log of the geometric mean growth
rate, and the x axis is the lag-year trend, i.e., for lag 10 gives the
10-year trend from 2014 to 2024. The black line is the posterior mean,
...
y-axis origin.}\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Posterior trend estimates for Steller's
...
the dashed black line is the posterior median, and the orange band is
the 95\% credible interval. The horizontal thick black line is the
y-axis origin.}\DIFaddendFL }

\end{figure}%

\subsection{Wavelet analysis}\label{wavelet-analysis-1}

A wavelet power spectrum clearly revealed a high \DIFdelbegin \DIFdel{``power '' }\DIFdelend \DIFaddbegin \DIFadd{power }\DIFaddend ridge across
...
15 year period. When the \DIFdelbegin \DIFdel{spectorgram }\DIFdelend \DIFaddbegin \DIFadd{spectrogram }\DIFaddend power is averaged across all years
...
simulation pvalue of p = 0.07, suggesting \DIFdelbegin \DIFdel{very }\DIFdelend weak evidence for a
longer period \DIFdelbegin \DIFdel{ocilation}\DIFdelend \DIFaddbegin \DIFadd{oscillation}\DIFaddend . When 500 posterior samples of the time series
were sampled from the GAM and a spect\DIFaddbegin \DIFadd{r}\DIFaddend ogram found for each sample, the
posterior \DIFaddbegin \DIFadd{mean }\DIFaddend maximum power was found at period \DIFdelbegin \DIFdel{(}\DIFdelend 6.52 \DIFdelbegin \DIFdel{) with credible
interval (}\DIFdelend \DIFaddbegin \DIFadd{(95\% }\DIFaddend CI: 6.23, 6.76), suggesting very high confidence in maximum average power at this period across the range of posterior sample\DIFaddbegin \DIFadd{s}\DIFaddend  of the GAM.

\begin{figure}

\caption{\label{fig-wavelet}\DIFdelbeginFL \DIFdelFL{Wavelet power spectrum for Steller's eider
across the ACP based on the posterior mean of the total population.
...
large.}\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Wavelet power spectrum for Steller's eider across the Arctic Coastal Plain.}} \newline{The power spectum was calculated at the posterior mean of the total population estimate.
Black points and lines give ridges where wavelet power is a local
maximum. White lines give a 95\% confidence interval for the period.
Larger power levels (red) correspond to greater probability density of
the wave period. The `cone of influence' is shown by the slightly
...
approximate integer multiples of the period (e.g., 6.5, 13.0, \DIFdelbegin \DIFdel{19.6}\DIFdelend \DIFaddbegin \DIFadd{19.5}\DIFaddend , and
so on) so that similar phases of the cycle are compared. For example, a
19-year trend calculated from 2003 to 2022 \DIFaddbegin \DIFadd{(approximately trough to trough) }\DIFaddend gives a posterior mean of 0.03 (\DIFaddbegin \DIFadd{95\% }\DIFaddend CI: -0.01, 0.09).
...
across time (Fig~\ref{fig-model}\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{C}\DIFaddend ).
...
size, population\DIFdelbegin \DIFdel{s}\DIFdelend  trends, or variation in density across space, then
...
applied for trend estimation (e.g. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{amundson2019}}\hskip0pt%DIFAUXCMD
,
\mbox{%DIFAUXCMD
\citep{rosenberg2019decline}}\hskip0pt%DIFAUXCMD
...
\citep{amundson2019, rosenberg2019decline, smith2021north}}\hskip0pt%DIFAUXCMD
}\DIFaddend ). Another advantage of the linear models used here (GAMs) is that interpretation and understanding is straightforward, where effects can be partitioned out into time, space, and space-by-time interaction effects (e.g.,
Fig~\ref{fig-model}).
...
Fig~\ref{fig-densityABR} to Fig~\DIFdelbegin \DIFdel{\ref{fig-combZoom}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-mapComb}C}\DIFaddend . In addition, the
...
south through time (Fig~\DIFdelbegin \DIFdel{\ref{fig-model-3}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-model}C}\DIFaddend ). The model also estimates the
temporal effect well with much better precision than the design-based
estimates (compare Fig~\DIFdelbegin \DIFdel{\ref{fig-abrA} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-model}B }\DIFaddend to Fig~\ref{fig-design}\DIFdelbegin \DIFdel{a}\DIFdelend \DIFaddbegin \DIFadd{A}\DIFaddend ).
...
(Fig~\ref{fig-design}\DIFdelbegin \DIFdel{b}\DIFdelend \DIFaddbegin \DIFadd{B}\DIFaddend ) and even model-based estimates, while much
better, are not precise (Fig~\ref{fig-yearACP}) compared to results from
the combined data. Thus, combining data sets allow\DIFaddbegin \DIFadd{s}\DIFaddend  the Triangle data to
...
Fig~\DIFdelbegin \DIFdel{\ref{fig-yearACP} shows that relatively few eiders are }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-yearComb} shows that nearly half the population might be found }\DIFaddend outside the Triangle area in \DIFdelbegin \DIFdel{most years, even though there are observations outside
the Triangle in some years(Fig~\ref{fig-observations})}\DIFdelend \DIFaddbegin \DIFadd{some years}\DIFaddend . Because the
space-time interaction of the Triangle-only model shows a decreasing
trend in the south (Fig~\DIFdelbegin \DIFdel{\ref{fig-model-3}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig-model}C}\DIFaddend ), there may be fewer eiders
...
The model and approach used here is similar to that of \DIFaddbegin \DIFadd{Amundson et al. 
}\DIFaddend \citep{amundson2019}, with two important simplifications. First, the
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{sauer1994observer} }\hskip0pt%DIFAUXCMD
\mbox{%DIFAUXCMD
\citep{fink2023double}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{sauer1994observer, fink2023double}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Therefore, observer
...
One interesting comparison to \DIFaddbegin \DIFadd{Amundson et al. }\DIFaddend \citep{amundson2019} is that they found very linear time trends for Steller's eider. This was also the case here when models included a spatio-temporal interaction and were fit to ACP-only data (model M2, Table~\ref{tbl-modelsACP}, results in
...
more complex model when confronted with sparse or mis\DIFdelbegin \DIFdel{s-}\DIFdelend matched data in
time and space.
...
for at least 25 years \DIFaddbegin \DIFadd{(Fig~\ref{fig-wavelet})}\DIFaddend . Thus, any measure of trend will show an increasing, decreasing, or stable estimate depending on the beginning and ending points for the trend calculation (Fig~\ref{fig-trendABR}, Fig~\ref{fig-trendComb}). Recovery criteria and species status
...
for trends so that similar points in the cycle are used (\DIFaddbegin \DIFadd{integer }\DIFaddend multiples of approximately 6.5). While each cycle is a stochastic realization that
...
period\DIFdelbegin \DIFdel{but perhaps varying amplitude}\DIFdelend .
...
estimates for long-tailed duck \DIFaddbegin \DIFadd{(}\emph{\DIFadd{Clangula hyemalis}}\DIFadd{) }\DIFaddend and did not account for non-detection by both observers (a `00' capture history). Analyses previous to 2019 (\DIFdelbegin \DIFdel{Bob
Stehn}\DIFdelend \DIFaddbegin \DIFadd{USFWS}\DIFaddend , unpublished) used a point estimate of 0.3 (SD = 0) based on expert judgement, which is remarkably close to the mean estimate used here, but did not include any uncertainty. Dunham and Grand \citep{dunham2017evaluating} used the same point estimate and allowed for some uncertainty (SD = 0.02). Therefore, the posterior mean
...
\label{s1-acpdata}
{\textbf{ACP data used in these analyses.\DIFdelbegin \DIFdel{The data can be found at
}%DIFDELCMD < \url{https://doi.org/10.7944/qtgn-y170} %%%
\DIFdel{\mbox{%DIFAUXCMD
\citep{osnasACP}}\hskip0pt%DIFAUXCMD
.}\DIFdelend } \DIFaddbegin \DIFadd{The data can be found at
}\url{https://doi.org/10.7944/qtgn-y170} \DIFadd{\mbox{%DIFAUXCMD
\citep{osnasACP}}\hskip0pt%DIFAUXCMD
.}\DIFaddend }

\paragraph*{S2 Triangle Data}
\label{s2-tridata}
{\textbf{Triangle data used for this report.\DIFdelbegin \DIFdel{The data is maintained by
the author and is available at
}%DIFDELCMD < \url{https://www.sciencebase.gov/catalog/item/6761f392d34e9734dd458940}%%%
\DIFdel{.
The Triangle data used in this report was obtained from ABR, Inc.~}\DIFdelend } \DIFaddbegin \DIFadd{The data is maintained by
the author and is available at
}\url{https://doi.org/10.7944/0x1v-4b77}\DIFadd{.
The Triangle data used in this report was obtained from ABR, Inc.~}\DIFaddend }

\paragraph*{S3 R Code}
\label{s3-code}
{\textbf{R code to reproduce these analyses, results, and this report.\DIFdelbegin \DIFdel{All code can be found at the GitHub repository,
}\DIFdelend \DIFaddbegin }
\DIFadd{All code can be found at the GitHub repository,
}\DIFaddend \url{https://github.com/USFWS/STEI-estimates}.\DIFdelbegin %DIFDELCMD < \MBLOCKRIGHTBRACE%%%
\DIFdelend }

\paragraph*{S4 \DIFdelbegin \DIFdel{Detection}\DIFdelend \DIFaddbegin \DIFadd{Table 3}\DIFaddend }
\label{s4-bradley}
{\textbf{\DIFdelbegin \DIFdel{A description of the field experiment and analysis to estimate
detection in }\DIFdelend \DIFaddbegin \DIFadd{Detection estimates by distance from transect.}} \DIFadd{This is an unpublished USFWS report that describes the field experiment and analysis to estimate detection probabilty of Steller's eider by an aerial observer in }\DIFaddend 2017 and 2018. \DIFdelbegin %DIFDELCMD < \MBLOCKRIGHTBRACE%%%
\DIFdelend \DIFaddbegin \DIFadd{Table 3 is used to calculate the detection probability averaged over distance.}\DIFaddend }

\paragraph*{S5 Results}
\label{s5-results}
{\textbf{R model objects, posteriors samples, figures, and csv files
summarizing various posteriors.\DIFdelbegin \DIFdel{All objects can be found at
}%DIFDELCMD < \url{https://www.sciencebase.gov/catalog/item/6761f393d34e9734dd458942}%%%
\DIFdel{.}\DIFdelend } \DIFaddbegin \DIFadd{All objects can be found at
}\url{https://doi.org/10.7944/22x5-t856}\DIFadd{.}\DIFaddend }
